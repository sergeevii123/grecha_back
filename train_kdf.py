...
for i in range(1000):
    cur_name = services_df[services_df['id_clustered'] == i]['Наименование_услуги'].value_counts().nlargest(1).index[0]
...
pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info['Дата_создания_записи']>='2016-01-01'].reset_index(drop=True)

# более ранний старт
# pupil_to_many_kdf_with_service_info = \
# pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info['Дата_создания_записи']>='2015-09-01'].reset_index(drop=True)

pupil_to_many_kdf_with_service_info = \
pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info['Дата_создания_записи']<'2020-01-01'].reset_index(drop=True)

pupil_to_many_kdf_with_service_info['id_организации'] = pupil_to_many_kdf_with_service_info['id_организации'].astype(int)
...
pupil_to_many_kdf_with_service_info['id_clustered'] = pupil_to_many_kdf_with_service_info['id_clustered'].astype(int)


pupil_to_many_kdf_with_service_info = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info.clustered_name != ' '].reset_index(drop=True)

pupil_to_many_kdf_with_service_info = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info.clustered_name != '4 часа в неделю'].reset_index(drop=True)

pupil_to_many_kdf_with_service_info = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info.clustered_name != 'уровень I'].reset_index(drop=True)

pupil_to_many_kdf_with_service_info = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info.clustered_name != 'уровень II'].reset_index(drop=True)

pupil_to_many_kdf_with_service_info = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info.clustered_name != '6 часов в неделю'].reset_index(drop=True)

pupil_to_many_kdf_with_service_info = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info.clustered_name != '(обновлённый универсальный шаблон)'].reset_index(drop=True)

pupil_to_many_kdf_with_service_info = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info.clustered_name != 'Шаблон общеразвивающей программы (4 часа занятий в неделю)'].reset_index(drop=True)

pupil_to_many_kdf_with_service_info.to_csv('finished_relations.csv',index=False)
...
train_df = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info['Дата_создания_записи']<'2019-12-01']
test_df = pupil_to_many_kdf_with_service_info[pupil_to_many_kdf_with_service_info['Дата_создания_записи']>='2020-12-01']
...
def prec_at_k(user_id: int, k: int = 10, debug=True):
    ...
    gt_ids = set(test_pairs[test_pairs[user_column] == user_id][item_column].tolist())
    real_services = []
    if debug:
        for id_ in gt_ids:
            real_services.append(
                services_df[services_df[item_column]==leservices.inverse_transform([id_])[0]]['clustered_name'].values[0])

        rec_services = []
        for id_ in rec_ids:
            rec_services.append(
                services_df[services_df[item_column]==leservices.inverse_transform([id_])[0]]['clustered_name'].values[0])
        print(f'real {real_services}')
        print(f'rec {rec_services}')
    return len(rec_ids & gt_ids) / k
